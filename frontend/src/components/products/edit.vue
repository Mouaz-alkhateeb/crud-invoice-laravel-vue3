...
const getProduct = async () => {
    let response = await axios.get(`http://localhost:8000/api/get_product/${productId.value}`);
...
let getImage = () => {
    let photo = 'http://localhost:8000/upload/image.png'

    if (form.value.photo) {
        if (form.value.photo.indexOf('base64') != -1) {
            photo = form.value.photo
        } else {
            photo = 'http://localhost:8000/upload/'+form.value.photo
        }
    }
...
const updateProduct = () => {
    ...
    axios.post(`http://localhost:8000/api/update_product/${form.value.id}`, formData)
        .then((response) => {
            form.value.name = '',
